...
if (!$columns && support("table")) {
	...
	if (!$result) {
		...
		$email_fields = array();
		echo "<form id='forma' action='' method='post' enctype='multipart/form-data'>\n";
		$rows = array();
			...
			if (+$limit && ($found_rows === false || $found_rows > $limit || $page)) {
				...
				if ($jush != "simpledb") {
					...
					echo (($found_rows === false ? count($rows) + 1 : $found_rows - $page * $limit) > $limit
						? ' <a href="' . h(remove_from_uri("page") . "&page=" . ($page + 1)) . '" onclick="return !selectLoadMore(this, ' . (+$limit) . ', \'' . lang('Loading') . '...\');" class="loadmore">' . lang('Rodyti daugiau') . '</a>'
						: ''
					);
				} else {
					echo lang('Page') . ":";
