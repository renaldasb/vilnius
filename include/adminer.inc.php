...
<?php
	...
	*/
	function selectLimitProcess() {
		return (isset($_GET["limit"]) ? $_GET["limit"] : "10");
	}
	...
	*/
	function tablesPrint($tables) {
		...
		foreach ($tables as $table => $status) {
			...
			echo '<li><a href="' . h(ME) . 'select=' . urlencode($table) . '"' . bold($_GET["select"] == $table || $_GET["edit"] == $table, "select") . ">" . $name . "</a> ";
